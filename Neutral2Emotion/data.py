...
class MeadDataset(data.Dataset):
    def __init__(self, root, flist, transform=None, transform_gray = None, preprocess=True, image_loader = default_image_loader, config = None, pickle_loader =default_pickle_loader,
    ...
    def __getitem__(self, index):
        ...
        ref_emc_label = int(video_sample[4])
        ref_int_label = int(video_sample[5])
        ...
        return {'V':video, 'EL':emc_label, 'IL':int_label, 'VR':video_ref, 'REL':ref_emc_label, 'RIL':ref_int_label}
...
class MeadTestDataset(data.Dataset):
    def __init__(self, root, flist, transform=None, preprocess=True, image_loader = default_image_loader, config = None, pickle_loader =default_pickle_loader,
    ...
    def __getitem__(self, index):
        ...
        return {'VR':video_ref, 'VN': video_name} #
...
def get_data_loader_list(config, train):
    ...
    if train == True:
        dataset = MeadDataset(config['root'], config['flist'], transform=transform, transform_gray = transform_gray, config=config['audio_load_size'])
        ...
        dataset = MeadTestDataset(config['root'], config['flist'], transform=transform, config=config['audio_load_size'])
        loader = DataLoader(dataset=dataset, batch_size=config['batch_size'], shuffle=train, drop_last=True, num_workers=config['num_workers'])
