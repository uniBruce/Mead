...
from networks import Unet_V9_384x384, FaceDiscriminator, Generator, Expression_Transformer, Unet_Int_384x384, Unet_Enc_384x384
...
class GanimationTrainer(nn.Module):
    def __init__(self, param):
        ...
        self.gan = Generator().apply(weights_init(param['init']))
        # self.audio2exp = Audio2Exp(param).apply(weights_init(param['init']))
        # self.encdec = Unet_V9_384x384().apply(weights_init(param['init']))
        self.vgg = load_vgg16(param['vgg_path'])
        ...
        model_gan_params = list(self.gan.parameters())
    ...
    def update_learning_rate(self):
        # if self.model_em_scheduler is not None:
        #     self.model_em_scheduler.step()
    ...
    # def criterion_ldmk(self,input, target):
    #
    #     return torch.mean(F.pairwise_distance(input.double(), target.double())).type(torch.FloatTensor).cuda()
    ...
    def forward(self, video_ref, video_heatmap, param):
        self.eval()
    ...
    def gan_update(self, video, emc_label, int_label, video_ref, ref_emc_label, ref_int_label, param):

        self.model_gan_opt.zero_grad()

        '''generate emotion image: bidirectional process Neutral-to-Emotion & Emotion-to-Neutral'''
        onehot_E_emc = OneHot_emc_label(emc_label)
        onehot_N_emc = OneHot_emc_label(ref_emc_label)
        onehot_E_int = OneHot_int_label(int_label)
        onehot_N_int = OneHot_int_label(ref_int_label)
        onehot_E = torch.cat([onehot_E_emc, onehot_E_int], dim=1)
        onehot_N = torch.cat([onehot_N_emc, onehot_N_int], dim=1)
        E2N_image = self.gan(video, onehot_N)
        N2E_image = self.gan(video_ref, onehot_E)

        '''loss part'''
        # content loss
        loss_content_E_f = self.vgg_loss(self.vgg, N2E_image, video,  param)
        loss_content_N_f = self.vgg_loss(self.vgg, E2N_image, video_ref, param)
        content_loss = loss_content_E_f + loss_content_N_f

        # reconstruction loss
        loss_recon_e = self.criterion_recon(N2E_image, video)
        loss_recon_n = self.criterion_recon(E2N_image, video_ref)
        recon_loss = loss_recon_e + loss_recon_n

        # emotion classification loss
        emc_score_e = self.unetenc(N2E_image)
        emc_score_n = self.unetenc(E2N_image)
        int_score_e = self.unetint(N2E_image)
        int_score_n = self.unetint(E2N_image)
        emc_loss_e = self.criterion_emc(emc_score_e, emc_label)
        emc_loss_n = self.criterion_emc(emc_score_n, ref_emc_label)
        int_loss_e = self.criterion_int(int_score_e, int_label)
        int_loss_n = self.criterion_int(int_score_n, ref_int_label)
        emc_loss = emc_loss_e + emc_loss_n
        int_loss = int_loss_e + int_loss_n

        # generator loss
        loss_e2n = self.calc_gen_loss(E2N_image)
        loss_n2e = self.calc_gen_loss(N2E_image)
        gen_loss = loss_e2n + loss_n2e

        gan_loss = gen_loss + content_loss + emc_loss + int_loss + param['rec_w']*recon_loss

        gan_loss.backward()
        self.model_gan_opt.step()

        return gen_loss, content_loss, emc_loss, int_loss, recon_loss, E2N_image, N2E_image

    def dis_update(self, input_fake1, input_real1, input_fake2, input_real2, param):
        self.model_dis_opt.zero_grad()
        # D loss
        self.loss_dis1 = param['gan_w'] * self.calc_dis_loss(input_fake1.detach(), input_real1)
        self.loss_dis2 = param['gan_w'] * self.calc_dis_loss(input_fake2.detach(), input_real2)
        self.loss_dis = self.loss_dis1 + self.loss_dis2
    ...
    def resume(self, checkpoint_dir, param):
        ...
        self.gan.load_state_dict(state_dict['gan'])
    ...
    def save(self, snapshot_dir, iterations):
        ...
        torch.save({'gan':self.gan.state_dict()}, model_gan_name)
